...
def kp_plot(pred_kp, gt_kp, file_name):
    # pred_kp is J x 3 in numpy
    fig = plt.figure()
    ax = Axes3D(fig)
    ax.scatter(pred_kp[:,0], pred_kp[:,1], pred_kp[:,2],c='r')
    ax.scatter(gt_kp[:,0], gt_kp[:,1], gt_kp[:,2],c='b')
    # plotting line

    for line in limbs: # omit the line plotting
        ax.plot3D([pred_kp[line[0],0],pred_kp[line[1],0]],[pred_kp[line[0],1],pred_kp[line[1],1]],[pred_kp[line[0],2],pred_kp[line[1],2]], c='r')
        ax.plot3D([gt_kp[line[0],0],gt_kp[line[1],0]],[gt_kp[line[0],1],gt_kp[line[1],1]],[gt_kp[line[0],2],gt_kp[line[1],2]], c='b')
